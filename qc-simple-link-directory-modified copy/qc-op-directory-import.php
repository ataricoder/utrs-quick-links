								...
								if( $tmpName != "" )
								{
									...
									while(($data = fgetcsv($file)) !== FALSE) 
									{
										...
										$baseData[$data[0]][] = array(
											...
											'qcopd_timelaps' => $laps,
											'qcopd_description'=> trim($data[29]),
											'qcopd_tags'=> trim($data[30]),
											'qcopd_new'=> trim($data[31]),
											'qcopd_featured'=> trim($data[32]),
		...
		while ( $export_query->have_posts() ) 
		{
			...
			if( count($lists) > 0 )
			{
				...
				foreach( $lists as $list )
				{
					...
					$innerArray[29] = $qcopd_description;
					$innerArray[30] = $qcopd_tags;
					$innerArray[31] = $qcopd_new;
					$innerArray[32] = $qcopd_featured;
