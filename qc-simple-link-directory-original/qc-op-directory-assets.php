...
function qcopd_check_for_shorcode(){
	...
	if ( is_singular() ) {
		...
		}elseif($post && strpos($post->post_content, 'sld-multipage-category' ) !== false ){
			add_action('wp_enqueue_scripts', 'qcopd_load_global_scripts');
...
function qcopd_load_global_scripts()
{
	...
	wp_enqueue_script( 'qcopd-sldcustom-2co-script', 'https://www.2checkout.com/checkout/api/2co.min.js', array(),'',true);
	...
	$filterType = sld_get_option( 'sld_filter_ptype' );
		...
		wp_enqueue_style( 'jq-slick-theme-css', QCOPD_ASSETS_URL . '/css/slick-theme.css', array(), '1.0.1');
		wp_enqueue_script( 'jq-slick.min-js', QCOPD_ASSETS_URL . '/js/slick.min.js', array('jquery'));

}
function qcsld_category_tab(){
	wp_enqueue_script( 'qcopd-custom1-script', QCOPD_ASSETS_URL . '/js/category-tab.js', array('jquery'),'',true);
}
...
function qcsld_admin_enqueue($hook) {
	...
	wp_enqueue_script( 'qcsld-admin-tagInput-js', QCOPD_ASSETS_URL . '/js/tagInput.js' );
	wp_enqueue_script( 'qcopd-sldcustom-common-script-admin', QCOPD_ASSETS_URL . '/js/sldcustomization-common.js', array('jquery'));
	
	//wp_enqueue_script( 'qcopd-sldcustom-common-select2', QCOPD_URL . 'inc/cmb/js/vendor/select2/select2.js', array('jquery'));
