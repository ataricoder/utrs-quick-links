...
define('QCOPD_DIR_CAT', dirname(__FILE__)."/category_template");
...
add_action( 'added_post_meta', 'sld_sync_on_product_save', 10, 4 );

function sld_sync_on_product_save( $meta_id, $post_id, $meta_key, $meta_value ) {
	global $wpdb;
    if ( $meta_key == 'qcopd_list_item01' ) { // we've been editing the post
		if(isset($meta_value['qcopd_other_list']) && $meta_value['qcopd_other_list']!=''){
			...
			if(isset($meta_value['qcopd_item_link']) && $meta_value['qcopd_item_link']!=''){
				...
				if(isset($meta_value['qcopd_image_from_link']) && $meta_value['qcopd_image_from_link']=='1'){
					...
					//Main functionality
						...
						$image = sld2_get_web_page("https://www.googleapis.com/pagespeedonline/v1/runPagespeed?url=".$meta_value['qcopd_item_link']."&screenshot=true");
