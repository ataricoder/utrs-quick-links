...
jQuery(document).ready(function($)
{
    ...
    $(".sld_search_filter").keyup(function(){
        ...
        $("#opd-list-holder ul li").each(function(){
            ...
            // If the list item does not contain the text phrase fade it out
            if ($(this).text().search(new RegExp(filter, "i")) < 0 && dataurl.search(new RegExp(filter, "i")) < 0 && dataTitleTxt.search(new RegExp(filter, "i")) < 0 && parentH3.search(new RegExp(filter, "i")) < 0 && dataSubtitle.search(new RegExp(filter, "i")) < 0 ) {
                $(this).fadeOut();
				$(this).removeClass("showMe");		
