...
function sld_show_category(){
	...
	if(!empty($cterms)){
		require QCOPD_DIR_CAT.'/'.$temp_style.'.php';
	}
}
...
function qcopd_directory_multipage_full_shortcode( $atts = array() ){
	...
	elseif(isset($wp_query->query_vars['sldlist']) && $wp_query->query_vars['sldlist']!=''){
		...
		if($id>0){
			...
			echo do_shortcode('[qcopd-directory mode="one" list_id="'.$id.'" style="'.$temp_style.'" column="3" upvote="on" search="true" item_count="on" orderby="'.$orderby.'" filterorderby="date" order="'.$order.'" filterorder="ASC" paginate_items="false" favorite="disable" tooltip="true" list_title_font_size="" item_orderby="'.$item_orderby.'" list_title_line_height="" title_font_size="" subtitle_font_size="" title_line_height="" subtitle_line_height="" filter_area="normal" topspacing="" multipage="true" '.$tag.' '.$favorite.']');
		}
		...
		<?php
		echo do_shortcode('[qcopd-directory category="'.$wp_query->query_vars['sldcat'].'" style="'.$temp_style.'" column="3" upvote="on" search="true" item_count="on" orderby="'.$orderby.'" filterorderby="date" order="'.$order.'" filterorder="ASC" paginate_items="false" favorite="disable" tooltip="false" list_title_font_size="" item_orderby="'.$item_orderby.'" list_title_line_height="" title_font_size="" subtitle_font_size="" title_line_height="" subtitle_line_height="" filter_area="normal" topspacing="" multipage="true" '.$tag.' '.$favorite.']');
