...
$errors= array();
	...
	if(isset($_FILES['sld_link_image']) and $_FILES['sld_link_image']['name']!=''){
	  ...
	  //$custom_name = strtolower(explode('.',$_FILES['sld_link_image']['name'])[0]);
		$tempvar = strtolower(explode('.',$_FILES['sld_link_image']['name']));
...
<form action="<?php echo $current_url.'/?action=entry'; ?>" method="POST" enctype="multipart/form-data">
	<ul class="sld_form-style-1 sld_width">
		...
		<li>
            <label><?php echo __('Generator info', 'qc-pd') ?> </label>
            <input type="button" id="sld_generate" class="" value="Generate" />
        </li>

        <li><label><?php echo __('Link Title', 'qc-opd') ?> <span class="sld_required">*</span></label><input type="text" name="item_title" id="sld_title" class="field-long sld_text_width" value="" required/></li>
		...
		<li>
			<label><?php echo __('Category', 'qc-opd') ?> </label>
