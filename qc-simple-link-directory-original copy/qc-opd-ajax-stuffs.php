...
<?php
			...
			echo '<a href="'.$unserialized['qcopd_item_link'].'" target="_blank" class="sld_single_button">'.$visit_page.'</a></div></div>';
...
<div class="fa-field-modal" id="fa-field-modal-tag" style="display:block">
  ...
  <input type="submit" id="sld_tag_select" name="submit" value="Add Tags" />
</div>
...
function qcopd_img_download_fnc(){
	...
	$blueprint = array(
		...
		$image = sld2_get_web_page("https://www.googleapis.com/pagespeedonline/v1/runPagespeed?url=".$url."&screenshot=true");
		...
		//$blueprint['imgurl'] = @wp_get_attachment_image_src($attach_id,'thumbnail')[0];
    $tempvar = @wp_get_attachment_image_src($attach_id,'thumbnail');
